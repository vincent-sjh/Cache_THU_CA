...
int main() {
    ...
    for (int num = 0; num < 20; num++) {
        ...
        for (register size_t blk = 0; blk < MATRIX_SIZE / BLOCK_SIZE; ++blk) {
            for (register size_t i = 0; i < MATRIX_SIZE; i++) {
                for (register size_t k = blk * BLOCK_SIZE; k < (blk + 1) * BLOCK_SIZE; k++) {
                    register int tmp = a[i][k];
                    for (register size_t j = 0; j < MATRIX_SIZE; j++)
                        d[i][j] += tmp * b[k][j];
                }
            }
        }
        */
        ...
        double ratio = (double)(finish - start) / (finish_new - start_new);
        cout << "time ratio of performance optimization : " << ratio << endl;
        
        if (ratio > 3) {
            score += 1;
            cout << "test " << (num + 1) << " passed" << endl;
        }else {
            cout << "test " << (num + 1) << " failed" << endl;
        }
        sum_init += (double)(finish - start) / CLOCKS_PER_SEC;
        sum_new += (double)(finish_new - start_new) / CLOCKS_PER_SEC;
        //cout << "time ratio of performance optimization : " << (double)(finish - start) / (finish_new - start_new) << endl;
    }
    cout << "score: " << score << "/20" << endl;
    cout << "average time spent for original method : " << sum_init / 20 << " s" << endl;
    cout << "average time spent for new method : " << sum_new / 20 << " s" << endl;
    cout << "average time ratio of performance optimization : " << sum_init / sum_new << endl;
    /*
    start_new = clock();
